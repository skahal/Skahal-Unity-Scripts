...
public abstract class SHPoolBase : MonoBehaviour
{
	...
	{
		var go = CreateObject();
		go.AddComponent<SHGameObjectInPool>();
	...
	private void EnableGameObject(GameObject go, Vector3 position)
	{
		...
		{
			ReleaseGameObject(go, AutoDisableTime);
		}
	}
	...
	{
		go.GetComponent<SHGameObjectInPool>()
			  .StartAutoDisable(AutoDisableTime, ReleaseGameObject);
	}
	...
	public void ReleaseGameObject(GameObject go)
	{
		lock (this)
		{
			go.GetComponent<SHGameObjectInPool>().CancelAutoDisable();
			DisableObject(go);
			go.name = Name + " (FREE)";
